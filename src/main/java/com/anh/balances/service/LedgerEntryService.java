...
import com.anh.balances.controller.member.dto.LedgerEntryForm;
import com.anh.balances.controller.member.dto.LedgerEntryListItem;
import com.anh.balances.controller.member.dto.LedgerEntrySearch;
import com.anh.balances.model.PageResult;
import com.anh.balances.model.entity.LedgerEntry;
import com.anh.balances.model.entity.LedgerEntryItem;
import com.anh.balances.model.entity.Member;
import com.anh.balances.model.entity.consts.BalanceType;
import com.anh.balances.model.entity.embeddables.LedgerEntryItemPk;
import com.anh.balances.model.entity.embeddables.LedgerEntryPk;
import com.anh.balances.model.repo.LedgerEntryItemRepo;
import com.anh.balances.model.repo.LedgerEntryRepo;
import com.anh.balances.model.repo.LedgerRepo;
...
public class LedgerEntryService {
	...
	private String insert(Member member, LedgerEntryForm form) {
		...
		var amount = form.getItems().stream()
				         ...
				         .reduce((a, b) -> a.add(b)).orElse(BigDecimal.ZERO); // reduce function works for  adding total amount
		
		entry.setLastAmount(lastAmount);
		entry.setAmount(amount);
		
		//Insert Ledger Entry
		entry = entryRepo.save(entry); //this returned object is managed stages
		
		//Insert Ledger Entry Item
	...
	private Function<CriteriaBuilder,CriteriaQuery<LedgerEntryListItem>> queryFunc(String username, BalanceType type, LedgerEntrySearch form) {
		return cb->{
			 var cq = cb.createQuery(LedgerEntryListItem.class);
			 var root = cq.from(LedgerEntry.class);
			 
			 LedgerEntryListItem.select(cq,root);
			 cq.where(form.where(username,type,cb,root));
	...
	private Function<CriteriaBuilder,CriteriaQuery<Long>> countFunc(String username, BalanceType type, LedgerEntrySearch form) {
		return cb->{
