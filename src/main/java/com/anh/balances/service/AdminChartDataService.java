...
public class AdminChartDataService {
	...
	private List<LineChartVo> loadYearlyData() {
        ...
        while(startMonth.compareTo(endMonth) <= 0) {
        	...
        	result.add(getData(date, from, to));  // searching for one month
	...
	private List<LineChartVo> loadMonthlyData() {
	   ...
	   while(startDate.compareTo(endDate) <= 0) {
		    result.add(getData(startDate ,startDate.atStartOfDay(), startDate.plusDays(1).atStartOfDay()));   //searching for one day
