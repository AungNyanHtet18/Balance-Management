...
public record MemberProfileDetails(
	...
	public static MemberProfileDetails  from(Member entity) { 
		...
		builder.id(entity.getId())
		        .name(entity.getName())
		        .profileImage(Optional.ofNullable(entity).map(a -> a.getProfileImage()).orElse("defaultprofile.png"))
		...
		String  address = list.stream().filter(StringUtils::hasLength)
    ...
    public static class Builder {
        ...
        public MemberProfileDetails build() {
            return new MemberProfileDetails(
                id, name, phone, profileImage, email, address, registeredAt, lastAccessAt
            );
        }
    }
}
