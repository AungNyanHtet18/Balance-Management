...
import com.anh.balances.model.entity.consts.Gender;
    ...
    public static ProfileEditForm from(Member entity) {
        return new Builder()
                ...
                .region(Optional.ofNullable(entity.getTownship())
                        .map(Township::getDistrict).map(District::getRegion).map(Region::getId).orElse(null))
                .district(Optional.ofNullable(entity.getTownship())
                		.map(Township::getDistrict).map(District::getId).orElse(null))
                .township(Optional.ofNullable(entity.getTownship())
                		 .map(Township::getId).orElse(null)) // assuming Township is an entity
                .address(entity.getAddress())
                .profileImage(Optional.ofNullable(entity.getProfileImage()).orElse("defaultprofile.png"))
                .build();
    }
